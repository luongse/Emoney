...
namespace MyUtility
{
    ...
    "\u00C1", "\u00C0"
        ...
        public static string UpperFirstCharacter(string strInput)
        {
            if (string.IsNullOrEmpty(strInput))
                return "";

            var splitInput = strInput.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            {
                var lowerInputCharacter = input.ToLower();
                var upperFirstCharacter = lowerInputCharacter[0].ToString().ToUpper() + lowerInputCharacter.Substring(1);
                if (countLoop > 0)
                    upperFirstCharacter = " " + upperFirstCharacter;

                stringBuilder.Append(upperFirstCharacter);
                countLoop++;
            }
